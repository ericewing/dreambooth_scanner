...
def main(args):
    ...
    # THIS IS WHERE CLASS IMAGES ARE GENERATED
    ...
    def save_weights(step):
        ...
        if accelerator.is_main_process:
            ...
            if args.save_sample_prompt is not None:
                with open(args.save_sample_prompt, "r") as f:
                    args.save_sample_prompts = json.load(f)

                sample_type_count=0
                for sample_prompt in args.save_sample_prompts:
                    ...
                    with torch.autocast("cuda"), torch.inference_mode():
                        for i in tqdm(range(args.n_save_sample), desc="Generating samples"):
                            ...
                            images[0].save(os.path.join(sample_dir, f"{str(sample_type_count).zfill(3)}{i}.png"))
                    if torch.cuda.is_available():
                        torch.cuda.empty_cache()
                del pipeline
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # The "not" works because we're targetting when the result of mod is '0' and '0' evaluates to false
